...
public class Arcade_Drive extends CommandBase {
    ...
    public void execute() {
        ...
        double leftJoyX = RobotContainer.j_stick_driver.getRawAxis(Constants.AXIS_LEFT_X);

        leftJoyY = LB_Pressed ? leftJoyY / 2 : leftJoyY;    // Apply Y
        leftJoyX = LB_Pressed ? leftJoyX / 2 : leftJoyX; // Apply X

        m_subsystem.arcadeDrive(leftJoyY, leftJoyX); // Run the subsystem method
    }
